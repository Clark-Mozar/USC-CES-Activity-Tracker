...
class GoogleCalendarApi { 
    ...
    public function CreateCalendarEvent($access_token, $calendar_id, $event_data, $all_day, $event_datetime, $event_timezone) { 
        ...
        $start_date = !empty($event_datetime['start_date'])?$event_datetime['start_date']:date("Y-m-d"); 
        $end_date = !empty($event_datetime['end_date'])?$event_datetime['end_date']:date("Y-m-d"); 
        ...
        if($all_day == 1){ 
            ...
            $dateTime_end = $end_date.'T'.$end_time.$timezone_offset; 
    ...
    public function GetCalendarEvents($access_token, $calendar_id) { 
        $url_parameters = array(); 

        $url_parameters['fields'] = 'items(id,summary,description,start,end)'; 
        $url_parameters['timeMin'] = date('c'); 

        $url_events = self::CALENDAR_EVENT . $calendar_id . '/events?' . http_build_query($url_parameters); 
        ...
        if ($http_code != 200) { 
            $error_msg = 'Failed to get events list'; 
            if (curl_errno($ch)) { 
                $error_msg = curl_error($ch); 
            } 
            throw new Exception('Error '.$http_code.': '.$error_msg); 
    ...
    function addMemberToGroup($memberEmail, $access_token) {
        $postBody = array(
            "email" => $memberEmail,
            "role" => "MEMBER"
        );
        ...
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, "https://www.googleapis.com/admin/directory/v1/groups/usccescebu@googlegroups.com/members");
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_POST, 1);
        ...
        if ($http_code == 200) {
            ...
            // error handling
            echo "Error adding member to group. HTTP status code: " . $http_code;
