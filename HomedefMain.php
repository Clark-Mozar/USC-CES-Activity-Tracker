                ...
                $datestart=$row['date_start'];
                    echo '<div class="dialogbox">';
                      echo '<div class="body">';
                        ...
                        echo 'Date & Time Start:<font style="background-color:white;" color="maroon"> '.date('F d, Y',strtotime($row['date_start']))." :".date('D', strtotime($datestart))." ".date('h:i A',strtotime($row['timestart'])).'</font><br>';
                        echo 'Date & Time End: <font style="background-color:white;" color="maroon"> '.date('F d, Y',strtotime($row['date_end']))." : ".date('D', strtotime($datestart))." ".date('D h:i A',strtotime($row['timeend'])).'</font><br>';
    ...
    <div class="scrollit">
     ...
     ?>
              ...
              <?php
                ...
                while($row = mysqli_fetch_assoc($result)){
                 $annid=$row['ann_id'];
                   $selectQueryC = "SELECT AN.ann_id,AN.ann_name,AN.ann_details,DATE_FORMAT(AN.dateposted,'%b %d, %Y : %a %h:%i %p') AS dateposted,U.firstName,U.lastName,AN.status,U.userType FROM tblannouncement AN LEFT JOIN tblusers U ON AN.uid=U.user_id WHERE AN.ann_id='$annid'";
